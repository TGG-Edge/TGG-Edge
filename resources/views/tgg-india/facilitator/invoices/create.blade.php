...
@section('title', 'Create Invoice | TGG Meta | TGG India')

@section('content')
<div class="admin-container">
    <h4 class="mb-3 trainer-heading">Create Invoice</h4>
    @include('tgg-india.layouts.includes.message')

    <div class="card p-3 mb-4">
        <form action="{{ route('tgg-india.facilitator.invoices.store') }}" method="POST">
            @csrf
                ...
                <select name="source_id" class="form-control">
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" 
                            {{ old('source_id', 1) == $user->id ? 'selected' : '' }}>
