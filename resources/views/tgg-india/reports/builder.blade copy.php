...
<div class="container" x-data="reportBuilder()" x-init="init()">
    ...
    <div class="card">
        <h3>Select Model</h3> <select x-model="model" @change="loadRelations()">
            <option value="">Select Model</option> <template x-for="(className,key) in models">
                <option :value="className" x-text="key"></option>
            </template>
        </select>
    </div> <!-- RELATIONS --> <template x-for="(relation,index) in relationLevels">
        <div class="card">
            <h3>Select Relation</h3> <select x-model="selectedRelations[index]" @change="loadNextRelation(index)">
                <option value="">Select Relation</option> <template x-for="(className,name) in relation">
    ...
    <div class="card">
        <h3>Report Types</h3>
        <div class="grid"> <label> <input type="checkbox" value="summary" x-model="reports"> Summary </label> <label>
                <input type="checkbox" value="table" x-model="reports"> Table </label> <label> <input type="checkbox"
                    value="bar_chart" x-model="reports"> Bar Chart </label> <label> <input type="checkbox"
                    value="line_chart" x-model="reports"> Line Chart </label> <label> <input type="checkbox"
                    value="pie_chart" x-model="reports"> Pie Chart </label> <label> <input type="checkbox"
                    value="top_records" x-model="reports"> Top Records </label> <label> <input type="checkbox"
                    value="comparison" x-model="reports"> Comparison </label> <label> <input type="checkbox"
                    value="timeline" x-model="reports"> Timeline </label> <label> <input type="checkbox" value="growth"
                    x-model="reports"> Growth </label> </div> <br> <button @click="generateReport()">Generate
            Report</button> <button onclick="window.print()">Print All Reports</button>
    </div> <!-- REPORT OUTPUT -->
                    ...
                    this.$nextTick(() => {
                        ...
                        if (data.bar_chart) {
                            ...
                            let options = {
                                ...
                                series: [{
                                    name: 'Total',
                                    data: data.bar_chart.map(i => i.total)
                                }],
