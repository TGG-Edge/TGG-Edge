...
@section('content')
    <div class="admin-container">
        ...
        <p>Hello <strong>{{ Auth::user()->name ?? 'User' }}</strong> (not
            <strong>{{ Auth::user()->name ?? 'User' }}</strong>? <a href="{{ route('tgg-india.logout') }}">Log out</a>)</p>

        <p><strong>WELCOME TO TGG-EDGE</strong></p>

        <p>It is a dynamic hub where ethical research meets grassroots action. This is where your inquiries, insights, and
            efforts converge to shape meaningful change through collaborative, well-coordinated projects.</p>

        <p>
            As a volunteer or researcher, you are part of a unified ecosystem committed to experiential learning, rigorous
            documentation, and outcome-oriented exploration. Here, you’ll find streamlined tools to manage assignments,
            exchange knowledge, and align your work with the broader values of sustainability, compassion, and community
            empowerment.
        </p>

        <p>Let’s co-create solutions that bridge theory and practice, deepen local impact, and contribute to a global
            narrative of self-reliance and human unity. Welcome aboard and onward with purpose.</p>
                        ...
                        <div class="card-body p-4">
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                ...
                                <span class="badge bg-{{ $item['color'] }}  text-{{ $item['color'] }}"
                                    style="--bs-bg-opacity: 0.2;">{{ strtoupper($item['title']) }} </span>
                            </div>
                            <h2 class="fw-bold text-dark mb-1">{{ $item['count'] }}</h2>
                            @if (isset($item['amount']))
                                @if ($item['title'] == 'Rewards')
                                    <p class="text-muted small mb-3">Total Points: {{ number_format($item['amount'], 0) }}
                                    </p>
                                @else
                                    <p class="text-muted small mb-3">Total Amount: ₹{{ number_format($item['amount'], 2) }}
                                    </p>
                                @endif
                            @else
        ...
        <div class="card border-0 shadow-sm rounded-4 mb-4">
            ...
            <div class="card-body">
                <p class="text-muted">
                    The dashboard provides a unified view of your contributions, incentives, rewards, and financial
                    transactions.
        ...
        <section class="vb-support-section rounded-4">
            <h3 class="card-header p-3 border-0 fw-semibold fs-5 text-center">Venture Bench Support</h3>
