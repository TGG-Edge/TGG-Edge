...
@section('title', 'Latest Announcements | TGG Meta | TGG India')

@section('content')
    <div class="admin-container">
        <h4 class="mb-3 showcase-heading">Latest Announcements Showcase</h4>
    ...
    @endif -->
        ...
        <div class="card p-3 mb-4">
            <form action="{{ route('tgg-india.admin.showcases.update') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="form_type" value="latest_announcements">

                <label class="form-label">Latest Announcements</label>
                <div id="opportunities-wrapper">
                    @php
                        $opportunities = $showcase->latest_announcements ?? [];
                    @endphp
                    @foreach ($opportunities as $i => $opportunity)
                        <div class="border p-2 mb-2 rounded opportunity-item">
                            <input type="text" name="latest_announcements[{{ $i }}][title]"
                                class="form-control mb-1" placeholder="Opportunity Title"
                                value="{{ $opportunity['title'] ?? '' }}">

                            <textarea name="latest_announcements[{{ $i }}][note]" class="js-ckeditor form-control mb-1"
                                placeholder="Opportunity Note">{{ $opportunity['note'] ?? '' }}</textarea>

                            <input type="url" name="latest_announcements[{{ $i }}][link]"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('add-opportunity').addEventListener('click', function () {
            ...
            div.innerHTML = `
                <input type="text" name="latest_announcements[${index}][title]" 
                    class="form-control mb-1" placeholder="Opportunity Title">

                <textarea name="latest_announcements[${index}][note]" 
