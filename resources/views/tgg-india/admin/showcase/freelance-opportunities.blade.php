    ...
    @endif -->
        ...
        <div class="card p-3 mb-4">
            <form action="{{ route('tgg-india.admin.showcases.update') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="form_type" value="freelance_opportunities">

                <label class="form-label">Freelancing Opportunities</label>
                <div id="opportunities-wrapper">
                    @php
                        $opportunities = $showcase->investment_opportunities ?? [];
                    @endphp
                    @foreach ($opportunities as $i => $opportunity)
                        <div class="border p-2 mb-2 rounded opportunity-item">
                            <input type="text" name="investment_opportunities[{{ $i }}][title]"
                                class="form-control mb-1" placeholder="Opportunity Title"
                                value="{{ $opportunity['title'] ?? '' }}">

                            <textarea name="investment_opportunities[{{ $i }}][note]" class="js-ckeditor form-control mb-1"
                                placeholder="Opportunity Note">{{ $opportunity['note'] ?? '' }}</textarea>

                            <input type="url" name="investment_opportunities[{{ $i }}][link]"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('add-opportunity').addEventListener('click', function () {
            ...
            div.innerHTML = `
                <input type="text" name="investment_opportunities[${index}][title]" 
                    class="form-control mb-1" placeholder="Opportunity Title">

                <textarea name="investment_opportunities[${index}][note]" 
                    class="js-ckeditor form-control mb-1" placeholder="Opportunity Note"></textarea>

                <input type="url" name="investment_opportunities[${index}][link]" 
                    class="form-control my-1" placeholder="Opportunity Link">
            `;
