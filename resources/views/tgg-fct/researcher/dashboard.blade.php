...
@section('title', 'Researcher Dashboard | Tgg Edge | Tgg Fct')

@section('content')
<div class="main-container">
    <div class="para-text">
        <p>Hello <strong>{{ Auth::user()->name ?? 'Researcher' }}</strong> (not <strong>{{ Auth::user()->name ?? 'Researcher' }}</strong>? <a href="{{ route('tgg-fct.logout') }}">Log out</a>)</p>
...
<div class="top-row" style="display: flex; gap: 20px; flex-wrap: wrap;">
   ...
   <div class="section update-box" style="flex: 1;">
    <form action="{{ route('tgg-fct.researcher.project.store') }}" method="POST" enctype="multipart/form-data">
        @csrf 
                ...
                @if($volunteer_applications->count() > 0)
                    @foreach($volunteer_applications as $volunteer_application)
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('tgg-fct.researcher.project-collaboration.application/accept-reject') }}" method="POST" style="">
                                    @csrf
...
">Researcher Name</th>
        ...
        <tbody>
            <form action="{{ route('tgg-fct.researcher.project-progress.update') }}" method="POST" enctype="multipart/form-data">
                <tr>
                @if($project)
                 @csrf
...
<div class="section full-width" style="margin-top: 40px;">
    ...
    <table class="researcher-table">
        ...
        <tbody>
            ...
            <form action="{{ route('tgg-fct.researcher.project-collaboration-progress.update') }}" method="POST">
                @csrf
